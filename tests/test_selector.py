...
"""
    ...
    def test_selector_init_with_etree_element(self):
        from lxml import etree

        element = etree.Element("root")
        sel = self.sscls(root=element)
        self.assertEqual(sel.type, "html")

    def test_invalid_json(self) -> None:
        invalid_json = "root"
        sel = self.sscls(text=invalid_json, type="json")
        self.assertEqual(sel.root, None)

    def test_invalid_selector_calls(self) -> None:
        json = '{"attrib":value}'
        sel = self.sscls(text=json, type="json")
    ...
    def test_xpath_selector_on_type_text(self) -> None:
        html = "<html></html>"
        sel = self.sscls(text=html, type="text")

        sel_list = sel.xpath("//html")
        self.assertEqual(sel_list[0].type, "html")

    def test_css_selector_on_type_text(self) -> None:
        html = "<html></html>"
        sel = self.sscls(text=html, type="text")
    ...
    def test_jsonpath_selectors(self) -> None:
        ...
        self.assertIsSelectorList(sel_list)
        self.assertEqual(sel_list.get(), '{"color": "red", "price": 19.95}')
        ...
        sel_list_empty = sel.jsonpath("$..contact")
        self.assertEqual(sel_list_empty, [])
